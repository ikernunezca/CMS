...
#This script will start a total of 1000 randomizations of the patients diagnosis. Please, set "phenotype" variable (line 4) depending on your desired analysis.
...
largestcomponents <- c()
amigosenlargestcomponents <- c()
amigoscausalesenlargestcomponents <- c()
amigosnocausalesenlargestcomponents <- c()
kante <- 0 #start iteration count

#Iteration bucle. Rerun from here in case of the script process getting stacked.
repeat{
  if(kante > 1000){
    message("Randomization Done. Toasty!")
    break()
  }
#randomize patient group assignment
...
repeat{
  ...
  vectorinho <- table(unlist(severedata[i,]))["2"] #8 times 2 copies
  if(is.na(vectorinho)){
    while(is.na(vectorinho)){
    i <- i+1
    vectorinho <- table(unlist(severedata[i,]))["2"]
    }
  }
  if(vectorinho == 8){
...
repeat{
  ...
  vectorinhod <- table(unlist(milddata[d,]))["2"]
...
#Retrieve communities of each gene from each study
  ...
  if(phenotype == "severe"){
  algenes <- c(severeonly,cosa,cnvsevereonly) #Severe-Only Randomized Analysis 
  }
  if(phenotype == "notsevere"){
  algenes <- c(notsevereonly,cosa,cnvsnotsevereonly) #To get a Mild-Only Randomized Analysis
  }
  ...
  g <- as.undirected(g) 
  conmu <- components(g)
  #To analyze the graph we count the number of genes of interest -'amigos','amigoscausales' & 'amigosnocausales' vectors-
  #that are in the largest component in each iteration.
  suc <- which(table(conmu$membership) == max(conmu$csize))
  nombrecillos <- names(which((conmu$membership)==suc))
  ekambi <- length(nombrecillos)
  if(phenotype == "severe"){
  amigos <- c("COL15A1","TNXB","HSPG2","LOXL3","AGRN","VCAN","CHGB","TNC","LAMA2","USH2A","PLEC","COL13A1","LRP4","LAMA5","LAMB2")
  amigosnocausales <- c("COL15A1","TNXB","HSPG2","LOXL3","VCAN","CHGB","TNC","LAMA2","USH2A") #Uncomment to test severe module and comment next line.  
  }
  else if(phenotype == "notsevere"){
  amigos <- c("LAMA5","LAMB2","PLEC","LRP4","AGRN","COL13A1","HMCN1","TLL2","MCAM","ADAM28","COL6A5","LAMB4","ADAMTS9","ITIH5","MSR1")
  amigosnocausales <- c("HMCN1","TLL2","MCAM","ADAM28","COL6A5","LAMB4","ADAMTS9","ITIH5","MSR1")#Uncomment to test mild module and comment previous line.
  }
  amigoscausales <- c("PLEC","COL13A1","LRP4","LAMA5","LAMB2","AGRN")
  bazingacausales <-  amigoscausales[which(amigoscausales %in% nombrecillos)]
  bazinganocausales <-  amigosnocausales[which(amigosnocausales %in% nombrecillos)]
  bazinga <- amigos[which(amigos %in% nombrecillos)]
  lenbazinga <- length(bazinga)
  lenbazingacausales <- length(bazingacausales)
  lenbazinganocausales <- length(bazinganocausales)
  largestcomponents <- c(largestcomponents,ekambi) #a vector with all largest component size of each iteration.
  amigosenlargestcomponents <- c(amigosenlargestcomponents,lenbazinga)  #a vector with all largest component sizes of each iteration.
  amigoscausalesenlargestcomponents <- c(amigoscausalesenlargestcomponents,lenbazingacausales)  #a vector with how many of the causal genes of our module are in the largest component in each iteration.
  amigosnocausalesenlargestcomponents <- c(amigosnocausalesenlargestcomponents,lenbazinganocausales) #a vector with how many of the non-causal genes of our module are in the largest component in each iteration.
  message(paste(kante," ",Sys.time())) #What time is it?
  print(paste(lenbazinga,lenbazingacausales,lenbazinganocausales))
  kante <- kante+1 
}



#Uncomment and configure next lines to Export a PDF archive and a csv with the graph structure/edges.
#pdf(file= "~/Desktop/Iker/SecondPart/Starplots/StarplotsWithCNVs/8sharedcommunity.pdf")   
#test.layout <- layout_(g,with_dh(weight.edge.lengths = edge_density(g)/1000))
#plot(g, layout = test.layout)
#dev.off()

#write.csv(x= as_edgelist(g),file= "~/Desktop/Iker/SecondPart/Starplots/StarplotsWithCNVs/SEVEREONLYFINALedgelist8cnvcompoundomim.csv",row.names=FALSE)
